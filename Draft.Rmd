...
>>>>>>> be74d972b2e7d780938ddd0f4c770b4fb322adb6
# Introduction
...
the `happiness` dataset was loaded, containing data on countries' economic, social, and political stability from the World Happiness Report. To ensure consistency across datasets, some country names were adjusted to match the naming conventions in the `world_map` dataset. This adjustment was necessary for accurate geographic mapping. After renaming, the `happiness` dataset was joined with `world_map` to incorporate latitude and longitude coordinates for each country, enabling the visualization of global data trends on a map. Additional geographic information was also added, including the distance of each country from the equator and hemisphere designation, by using centroids of each country's polygon in the `world` dataset. This enhancement facilitates spatial analysis based on proximity to the equator and hemisphere differences.
...
```{r insight2, fig.height=4, fig.width=6, fig.cap="Boxplot of perception of corruption vs total COVID cases by continent"}
