...
cyclones_sf <- st_as_sf(cyclones_data, coords = c("long", "lat"), crs = 4326) #convert coords to sf object
world_sf <- ne_countries(scale = "medium", returnclass = "sf") #converts sf object to world map

cyclone_countries <- st_join(cyclones_sf, world_sf, join = st_intersects) #joins the cyclones data to the world map
 
countries_with_cyclones <- cyclone_countries %>%
  group_by(admin) %>%
  mutate(
    storm_id = cumsum(c(TRUE, diff(Number) != 0)),
    num_unique_storms = n_distinct(storm_id)
  ) %>%
  ungroup() %>%
  distinct(admin, num_unique_storms) %>%
  filter(!is.na(admin))
