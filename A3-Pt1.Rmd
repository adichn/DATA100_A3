...
author: "Put your group number from MyLS here: 8"
...
convert_latlon <- function(latlon) {
  ...
  parsed_num <- parse_number(latlon)
    if_else(str_detect(latlon, "[SW]"), -parsed_num, parsed_num)
}
...
cyclones_data_update_2 <- cyclones_data_update_1 |>
    mutate(
        date = make_datetime(ObservYear, Month, Day, Hour * 100 + Minute)
    )
...
aware_pop <- inner_join(
  ...
  pop_data,
  by = "country"
) |> summarise(country, "total_population" = SP.POP.TOTL, aware_no, aware_alittle, aware_moderate, aware_alot, aware_refuse)
...
covid_2020_match_country <- left_join(aware_pop, covid_2020, by = "country")
...
aware_covid <- inner_join(aware_pop, covid_2020_match_country) |>
  group_by(country) |>
  mutate(
    "cases_per_capita" = total_cases / total_population
  )
