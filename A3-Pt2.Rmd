...
# Assuming 'name' indicates 'min' or 'max' and 'value' contains the ice extent measurements
ggplot(data=ice_extent_yearly, aes(x = year, color = name, linetype = region)) +
...
storm_ice_comparison <- inner_join(storm_summary, ice_extent_yearly, by = "year")


print(storm_ice_comparison)

ggplot(storm_ice_comparison, aes(x = total_storms, y = value, color = region)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    facet_wrap(~name) +
    labs(
        title = "Storm Frequency vs. Sea Ice Extent",
        x = "Total Number of Storms",
        y = "Sea Ice Extent (Million sq. km)",
        color = "Region"
    )

ggplot(storm_ice_comparison, aes(x = max_wind_speed, y = value, color = region)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE) +
    facet_wrap(~name) +
    labs(
        title = "Maximum Storm Intensity vs. Sea Ice Extent",
