...
cyclones_sf <- st_as_sf(cyclones_data, coords = c("long", "lat"), crs = 4326)
world_sf <- ne_countries(scale = "medium", returnclass = "sf")
cyclone_countries <- st_join(cyclones_sf, world_sf, join = st_intersects)
countries_with_cyclones <- cyclone_countries %>%
  distinct(admin) %>%
  filter(!is.na(admin))
...
cyclone_affected_countries <- climate_awareness %>%
  filter(country %in% countries_with_cyclones$admin)

cyclone_unaffected_countries <- climate_awareness %>%
  filter(!country %in% countries_with_cyclones$admin)

climate_awareness_summary <- bind_rows(
  cyclone_affected_countries |> mutate(group = "Affected"),
  cyclone_unaffected_countries |> mutate(group = "Unaffected")
)


climate_awareness_long <- climate_awareness_summary |>
  select(-aware_base) |>
  pivot_longer(cols = starts_with("aware_"), names_to = "awareness_level", values_to = "average")


ggplot(climate_awareness_long, aes(x = awareness_level, y = average, fill = group)) +
