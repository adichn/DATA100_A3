...
countries_with_cyclone
countries_without_cyclone
...
climate_affected <- climate_awareness |>
  filter(country %in% countries_with_cyclone$country) |>
  left_join(countries_with_cyclone, by = "country") |>
  rename(cyclone_count = count) |>
  mutate(
    affected = "Affected",
    weighting = cyclone_count / sum(cyclone_count, na.rm = TRUE)
  )

climate_unaffected <- climate_awareness |>
  filter(country %in% countries_without_cyclone$country) |>
  mutate(affected = "Unaffected", weighting = 1)

climate_comparison <- bind_rows(climate_affected, climate_unaffected) |>
  pivot_longer(
    cols = c(aware_alot, aware_moderate, aware_alittle, aware_refuse),
    names_to = "awareness_category",
    values_to = "value"
  ) |> mutate(awareness_category = factor(
    awareness_category,
    levels = c("aware_alot", "aware_moderate", "aware_alittle", "aware_refuse")
  ))

awareness_averages <- climate_comparison |>
  group_by(awareness_category, affected) |>
  summarise(
    avg_value = ifelse(affected == "Affected", 
                       weighted.mean(value, w = weighting, na.rm = TRUE),
                       mean(value, na.rm = TRUE)),
    .groups = "drop"
  )

ggplot(awareness_averages, aes(x = awareness_category, y = avg_value, fill = affected)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_discrete(name = "Storm Impact") +
  scale_x_discrete(labels = c(
    "aware_alot" = "Aware a Lot",
    "aware_moderate" = "Moderately Aware",
    "aware_alittle" = "Aware a Little",
    "aware_refuse" = "Refused Awareness"
  )) +
  labs(
    title = "Average Climate Awareness Levels in Cyclone Affected and Unaffected Countries",
    x = "Awareness Category",
    y = "Average Awareness Level"
  ) +
  theme_minimal()
...
conbined_data <- full_join(covid_2020, happiness, by = "country") |>
  ungroup()|>
  select(where(is.numeric))

cor_matrix <- cor(conbined_data, use = "complete.obs")
cor_matrix
