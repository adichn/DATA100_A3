...
We explored many aspects of the data, but will demonstrate three. These are \<\<Social Support VS Life Expectancy\>\>, \<\<Perception of Corruption VS Covid Cases\>\>, and \<<Freedom to Make Life Choices & Climate Awareness (OR DISTANCE VS COVID)>\>
...
joint_data <- inner_join(climate_awareness, happiness, by = "country")|>
  
  select(country, aware_no, aware_alittle, aware_moderate, aware_alot, aware_refuse,  freedom_to_make_life_choices)|>
  
  pivot_longer(cols  = c(aware_no, aware_alittle, aware_moderate, aware_alot, aware_refuse, ), names_to =  "awareness", values_to = "aware_percent")|>  
  ...
  ggplot()+
    ...
    geom_point()+
    geom_smooth( method = "lm", color = "red", se = TRUE, size = 0.75) +
    facet_wrap(~awareness)+
    labs(
    title = "Freedom to Make Life Choices and Awareness by Awareness Type",
    x = "Awareness (%)",
