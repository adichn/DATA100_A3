...
happiness_yearly <- happiness_data |>
  ...
  summarize(avg_life_ladder = mean(life_ladder, na.rm = TRUE),
            avg_log_gdp = mean(log_gdp_per_capita, na.rm = TRUE),
            avg_social_support = mean(social_support, na.rm = TRUE),
            avg_healthy_life_expectancy = mean(healthy_life_expectancy_at_birth, na.rm = TRUE))
...
p3 <- ggplot(combined_data, aes(x = avg_life_ladder, y = arctic_min_extent)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", color = "red", se = FALSE) +
  labs(title = "Correlation between Life Ladder and Arctic Ice Extent",
       x = "Average Life Ladder (Happiness)",
       y = "Arctic Minimum Ice Extent (million sq km)") +
  theme_minimal()

p4 <- ggplot(combined_data, aes(x = avg_life_ladder, y = antarctic_min_extent)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", color = "red", se = FALSE) +
  labs(title = "Correlation between Life Ladder and Antarctic Ice Extent",
       x = "Average Life Ladder (Happiness)",
